...
RUN dnf install --repofrompath=centos-appstream,http://mirror.stream.centos.org/9-stream/AppStream/x86_64/os/ --repofrompath=centos,http://mirror.stream.centos.org/9-stream/BaseOS/x86_64/os --disablerepo=* --enablerepo=centos dovecot && dnf clean all -y
